...
fn tab_width(layout: &str, ws: &crate::workspace::Workspace, tab_idx: usize) -> u16 {
    display_width_u16(&tab_chrome_label(layout, ws, tab_idx))
        .saturating_add(4)
        .max(MIN_TAB_WIDTH)
}

fn tab_chrome_label(layout: &str, ws: &crate::workspace::Workspace, tab_idx: usize) -> String {
    let name = if layout == "{index}" || layout.is_empty() {
        // Fast path: bare index (no name lookup).
        (tab_idx + 1).to_string()
    } else if let Ok(idents) = crate::tab_display::parse_tab_layout(layout) {
        ws.format_tab_label(tab_idx, &idents)
    } else {
        // Parse failure → fallback to default.
        ws.tab_display_name(tab_idx)
            .unwrap_or_else(|| (tab_idx + 1).to_string())
    };
    let zoom_suffix = if ws.tabs.get(tab_idx).is_some_and(|tab| tab.zoomed) { " Z" } else { "" };
    format!("{name}{zoom_suffix}")
}

fn layout_tab_hit_areas(layout: &str, ws: &crate::workspace::Workspace, area: Rect, scroll: usize) -> Vec<Rect> {
    ...
    for (idx, rect) in rects.iter_mut().enumerate().skip(scroll) {
        ...
        let desired = tab_width(layout, ws, idx);
...
fn centered_tab_scroll(layout: &str, ws: &crate::workspace::Workspace, area: Rect) -> usize {
    ...
    for scroll in 0..=ws.active_tab {
        let rects = layout_tab_hit_areas("{index}: {name}", ws, area, scroll);
...
fn max_tab_scroll(ws: &crate::workspace::Workspace, area: Rect) -> usize {
    (0..ws.tabs.len())
        .find(|&scroll| {
            layout_tab_hit_areas("{index}: {name}", ws, area, scroll)
                .last()
...
pub(crate) fn compute_tab_bar_view(
    ...
    if !mouse_chrome {
        ...
        let scroll = if follow_active {
            centered_tab_scroll(layout, ws, area).min(max_scroll)
        } else {
            current_scroll.min(max_scroll)
        };
        return TabBarView {
            scroll,
            tab_hit_areas: layout_tab_hit_areas(layout, ws, area, scroll),
    ...
    let all_tabs = layout_tab_hit_areas(layout, ws, all_tabs_area, 0);
    ...
    let scroll = if follow_active {
        centered_tab_scroll(layout, ws, tab_area).min(max_scroll)
    } else {
        current_scroll.min(max_scroll)
    };
    let tab_hit_areas = layout_tab_hit_areas(layout, ws, tab_area, scroll);
...
pub(super) fn render_tab_bar(app: &AppState, frame: &mut Frame, area: Rect) {
    ...
    // Parse the configured tab display layout once per render.
    let layout_string = &app.keybinds.tab_display_layout;
    ...
    for (idx, tab) in ws.tabs.iter().enumerate() {
        ...
        let name = tab_chrome_label(layout_string, ws, idx);
...
mod tests {
    ...
    fn tab_bar_marks_zoomed_tabs_without_renaming_them() {
        ...
        let view = compute_tab_bar_view("{index}: {name}", &app.workspaces[0], app.view.tab_bar_rect, 0, true, false);
    ...
    fn zoom_marker_counts_toward_tab_width() {
        ...
        assert_eq!(tab_width("{index}: {name}", &ws, 0), 17);
    }
