        ...
        match action {
            ...
            NavigateAction::NewTab => {
                if self.state.active.is_some() {
                    if self.state.prompt_new_tab_name {
                        ...
                        let cwd = self.state.requested_new_tab_cwd.take().map(|p| p.to_string_lossy().to_string());
                        self.dispatch_tui_api_request(
                            "tui.key.tab.create",
            ...
            NavigateAction::NewTabHome => {
                if self.state.active.is_some() {
                    ...
                    let cwd = home.map(|p| p.to_string_lossy().to_string());
                    self.dispatch_tui_api_request(
                        "tui.key.tab.create",
                        crate::api::schema::Method::TabCreate(
                            crate::api::schema::TabCreateParams {
...
pub(super) fn execute_navigate_action_in_context(
    ...
    match action {
        ...
        NavigateAction::NewTabHome => {
            if state.active.is_some() {
                let home = std::env::var_os("HOME").map(std::path::PathBuf::from);
                state.requested_new_tab_cwd = home;
                state.request_new_tab = true;
                leave_navigate_mode(state);
            }
        }
    ...
    for (bindings, action) in [(&state.keybinds.last_pane, NavigateAction::LastPane)] {
        for binding in &bindings.bindings {
            if binding.trigger.is_prefix()
                && terminal_key_matches_combo(prefix_key.clone(), binding.trigger.combo())
            {
                return Some(action);
            }
        }
